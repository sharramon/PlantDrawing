...
namespace TiltBrush
{
    ...
    public class Stroke : StrokeData
    {
        ...
        public SimpleCanvas m_IntendedCanvas;
        /// Selected strokes need to remember which canvas they came from
        public SimpleCanvas m_PreviousCanvas;
        ...
        public void CopyGeometry(SimpleCanvas targetCanvas, Stroke baseStroke)
        {
            if (m_Type != Type.NotCreated)
            {
        ...
        /// be parented to an arbitrary Transform, only to SimpleCanvas.
        public void SetParent(SimpleCanvas canvas)
        {
            SimpleCanvas prevCanvas = Canvas;
        ...
        /// Directly analagous to Transform.SetParent, except strokes may not
        /// be parented to an arbitrary Transform, only to SimpleCanvas.
        public void SetParentKeepWorldPosition(SimpleCanvas canvas, TrTransform? leftTransform = null)
        {
            SimpleCanvas prevCanvas = Canvas;
            if (prevCanvas == canvas)
            {
            ...
            bool bWasTransformed = leftTransform.HasValue &&
                !TrTransform.Approximately(SimpleApp.Instance.ActiveCanvas.Pose, leftTransform.Value);
