...
namespace TiltBrush
{
    ...
    public abstract class BaseBrushScript : MonoBehaviour
    {
            ...
            BrushDescriptor desc, Color color, float size_PS)
        {
            ...
            // Simplified: use direct transform instead of Coords
            xfInParentSpace.ToLocalTransform(line.transform);
        ...
        {
            // Simplified: use basic cost calculation without complex quality levels
            // Original: (verts / 6) * QualityControls.m_Instance.AppQualityLevels.GetWeightForBrush(desc.m_Guid) * size;
            return (verts / 6) * size;
        }
        ...
        public TrTransform m_LastSpawnXf { get; set; }
        ...
        public SimpleCanvas Canvas
        {
            get
            {
                ...
                return transform.parent.GetComponent<SimpleCanvas>();
            }
        }
        ...
        /// <summary>
        /// Applies stroke simplification to reduce geometry complexity for performance.
        /// Override in derived classes to implement specific simplification logic.
        /// </summary>
        /// <param name="simplificationFactor">0.0 = no simplification, 1.0 = maximum simplification</param>
        public virtual void ApplyStrokeSimplification(float simplificationFactor)
        {
            // Base implementation - override in derived classes for specific simplification
            if (simplificationFactor <= 0f) return;
            
            // Default behavior: reduce vertex count based on simplification factor
            // This is a placeholder - actual implementation depends on the brush type
            Debug.Log($"Applying stroke simplification factor {simplificationFactor} to {name}");
        }
