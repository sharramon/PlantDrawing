...
namespace TiltBrush
{
    ...
    public abstract class QuadStripBrush : BaseBrushScript
    {
        ...
        // Flag to track if this is the first call to UpdatePositionImpl
        protected bool m_IsFirstCall = true;
        ...
        {
            return kSolidMinLengthMeters_PS * SimpleApp.METERS_TO_UNITS * POINTER_TO_LOCAL +
        ...
        protected override void InitBrush(BrushDescriptor desc, TrTransform localPointerXf)
        {
            ...
            Debug.Log($"QuadStripBrush: InitBrush - Assigned mesh with {m_Geometry.m_Vertices.Length} vertices and {m_Geometry.m_Tris.Length} triangles");
            Debug.Log($"QuadStripBrush: InitBrush - First 6 vertices: {m_Geometry.m_Vertices[0]}, {m_Geometry.m_Vertices[1]}, {m_Geometry.m_Vertices[2]}, {m_Geometry.m_Vertices[3]}, {m_Geometry.m_Vertices[4]}, {m_Geometry.m_Vertices[5]}");
            Debug.Log($"QuadStripBrush: InitBrush - First 6 triangles: {m_Geometry.m_Tris[0]}, {m_Geometry.m_Tris[1]}, {m_Geometry.m_Tris[2]}, {m_Geometry.m_Tris[3]}, {m_Geometry.m_Tris[4]}, {m_Geometry.m_Tris[5]}");
            Debug.Log($"QuadStripBrush: InitBrush - First 6 colors (alpha): {m_Geometry.m_Colors[0].a}, {m_Geometry.m_Colors[1].a}, {m_Geometry.m_Colors[2].a}, {m_Geometry.m_Colors[3].a}, {m_Geometry.m_Colors[4].a}, {m_Geometry.m_Colors[5].a}");
        }
            ...
            out int earliestChangedQuad)
        {
            ...
            // Don't create any geometry if we're not generating a new quad
            if (!bGenerateNew)
            {
        ...
        float GetSmoothedPressure(float pressure01, Vector3 pos)
        {
            ...
            float distanceM = Vector3.Distance(m_LastSpawnPos, pos) * SimpleApp.UNITS_TO_METERS;
        ...
        override public void ApplyChangesToVisuals()
        {
            ...
            Debug.Log($"QuadStripBrush: ApplyChangesToVisuals - Updated mesh with {geometry.m_Vertices.Length} vertices, m_LeadingQuadIndex={m_LeadingQuadIndex}, GetNumUsedVerts()={GetNumUsedVerts()}");
            if (m_LeadingQuadIndex > 0)
            {
                Debug.Log($"QuadStripBrush: ApplyChangesToVisuals - First 6 vertices: {geometry.m_Vertices[0]}, {geometry.m_Vertices[1]}, {geometry.m_Vertices[2]}, {geometry.m_Vertices[3]}, {geometry.m_Vertices[4]}, {geometry.m_Vertices[5]}");
                Debug.Log($"QuadStripBrush: ApplyChangesToVisuals - First 6 colors (alpha): {geometry.m_Colors[0].a}, {geometry.m_Colors[1].a}, {geometry.m_Colors[2].a}, {geometry.m_Colors[3].a}, {geometry.m_Colors[4].a}, {geometry.m_Colors[5].a}");
            }
        }
            ...
            Vector3 vPos, Quaternion ori, float fPressure)
        {
            ...
            // Handle first call - just save initial position without creating mesh
            if (m_IsFirstCall)
            {
                m_IsFirstCall = false;
                m_LastSpawnXf = TrTransform.TRS(vPos, Quaternion.identity, 1f); // Save the initial position
            ...
            if (moveLength < kMinimumMoveLengthMeters_PS * SimpleApp.METERS_TO_UNITS * POINTER_TO_LOCAL)
            {
                UnityEngine.Profiling.Profiler.EndSample();
                return false;
            }
            ...
            bool bIsBreak = false;
            if (m_AllowStripBreak && !m_PreviewMode)
            {
            ...
            Debug.Log($"QuadStripBrush: UpdatePositionImpl - moveLength={moveLength}, fSpawnInterval={fSpawnInterval}, bGenerateNewQuad={bGenerateNewQuad}");
            Debug.Log($"QuadStripBrush: UpdatePositionImpl - vPos={vPos}, m_LastSpawnPos={m_LastSpawnPos}");
            ...
            // Only assert if we actually generated a new quad
            if (bGenerateNewQuad)
            {
            ...
            {
                Debug.Assert(m_LeadingQuadIndex == iPrevLeadingIndex);
            }
