...
namespace TiltBrush
{
    ...
    class ThickGeometryBrush : GeometryBrush
    {
        ...
        {
            // Require actual movement before creating geometry
            // Only create geometry when there's meaningful movement
            return Mathf.Max(
                kSolidMinLengthMeters_PS * SimpleApp.METERS_TO_UNITS * POINTER_TO_LOCAL,
                PressuredSize(pressure01) * kSolidAspectRatio * 2f  // Require more movement
            );
        }
        ...
        {
            float minMove = kMinMoveLengthMeters_PS * SimpleApp.METERS_TO_UNITS * POINTER_TO_LOCAL;
            ...
            for (int iKnot = iKnot0; iKnot < m_knots.Count; ++iKnot)
            {
                ...
                // Additional check: if knots are at the same position, don't create geometry
                if (cur.length < 0.001f) // Very small threshold for "same position"
                {
                    shouldBreak = true;
                }
            ...
            for (int iKnot = iKnot0; iKnot < m_knots.Count; ++iKnot)
            {
                ...
                if (cur.HasGeometry)
                {
                    ...
                    if (isStart)
                    {
                        ...
                        // Use Vector3.zero since brush stroke GameObject is already positioned at prev.point.m_Pos
                        SetVert(cur.iVert, BRT, Vector3.zero + r, nUp, m_Color, alpha);
                        SetVert(cur.iVert, BRB, Vector3.zero + r, -nUp, m_Color, alpha);

                        SetVert(cur.iVert, BMT, Vector3.zero, nUp, m_Color, alpha);
                        SetVert(cur.iVert, BMB, Vector3.zero, -nUp, m_Color, alpha);

                        SetVert(cur.iVert, BLT, Vector3.zero - r, nUp, m_Color, alpha);
                        SetVert(cur.iVert, BLB, Vector3.zero - r, -nUp, m_Color, alpha);
                    }
