...
namespace TiltBrush
{
    ...
    public class MasterBrush : IPoolable
    {
        ...
        public MasterBrush()
        {
            ...
            // Initialize the list with the required capacity instead of using SetCount
            for (int i = 0; i < m_NumVerts; ++i)
            {
                m_UVWs.Add(Vector3.zero);
            }
            m_Colors = new Color32[m_NumVerts];
            m_Tangents = new Vector4[m_NumVerts];

            Debug.Log($"MasterBrush: Creating {numQuads} quads with {m_NumVerts} total vertices");
            ...
            for (int i = 0; i < numQuads; ++i)
            {
                ...
                for (int j = 0; j < 6; ++j)
                {
                    ...
                    m_Colors[iVertIndex] = new Color32(255, 255, 255, 0); // Set alpha to 0 for invisible initial geometry
                ...
                if (i < 3)
                {
                    Debug.Log($"MasterBrush: Quad {i} - Vertices: {m_Vertices[iVertGroupIndex]}, {m_Vertices[iVertGroupIndex + 1]}, {m_Vertices[iVertGroupIndex + 2]}, {m_Vertices[iVertGroupIndex + 3]}, {m_Vertices[iVertGroupIndex + 4]}, {m_Vertices[iVertGroupIndex + 5]}");
                    Debug.Log($"MasterBrush: Quad {i} - Triangles: {m_Tris[iVertGroupIndex]}, {m_Tris[iVertGroupIndex + 1]}, {m_Tris[iVertGroupIndex + 2]}, {m_Tris[iVertGroupIndex + 3]}, {m_Tris[iVertGroupIndex + 4]}, {m_Tris[iVertGroupIndex + 5]}");
                    Debug.Log($"MasterBrush: Quad {i} - Colors (alpha): {m_Colors[iVertGroupIndex].a}, {m_Colors[iVertGroupIndex + 1].a}, {m_Colors[iVertGroupIndex + 2].a}, {m_Colors[iVertGroupIndex + 3].a}, {m_Colors[iVertGroupIndex + 4].a}, {m_Colors[iVertGroupIndex + 5].a}");
                }
            }
            
            Debug.Log($"MasterBrush: Initial mesh created with {m_NumVerts} vertices, all at Vector3.zero with alpha=0");
        }
