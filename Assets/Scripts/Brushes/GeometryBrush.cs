...
namespace TiltBrush
{
    ...
    public abstract class GeometryBrush : BaseBrushScript
    {
        ...
        protected bool m_HasHadMeaningfulMovement = false;
        ...
        override public void ResetBrushForPreview(TrTransform localPointerXf)
        {
            ...
            // Only add one initial knot for preview too
            m_knots.Add(knot);
        }
        ...
        protected override void InitBrush(BrushDescriptor desc, TrTransform localPointerXf)
        {
            ...
            // Only add one initial knot - the second will be added on first update
            m_knots.Add(knot);
        ...
        override public void ApplyChangesToVisuals()
        {
            ...
            // Only create geometry if we've had meaningful movement
            if (m_FirstChangedControlPoint != null && m_HasHadMeaningfulMovement)
            {
                try
                {
        ...
        {
            // Simplified: no undo animation for Quest 3
            // Original: var rMeshScript = clone.AddComponent<UndoMeshAnimScript>();
            // rMeshScript.Init();
        }

        override protected bool UpdatePositionImpl(Vector3 pos, Quaternion ori, float pressure)
        {
            // Handle the case where we only have one knot initially
            if (m_knots.Count == 1)
            {
                // Check if we've moved enough to warrant adding a second knot
                float distance = Vector3.Distance(m_knots[0].point.m_Pos, pos);
                float minDistance = GetSpawnInterval(pressure) * 0.5f; // Require some movement
                
                if (distance < minDistance)
                {
                    // Not enough movement, just update the first knot's pressure
                    Knot firstKnot = m_knots[0];
                    firstKnot.point.m_Pressure = pressure;
                    firstKnot.smoothedPressure = pressure;
                    m_knots[0] = firstKnot;
                    return false; // Don't add another knot yet
                }
                
                // Add a second knot at the current position
                Knot secondKnot = m_knots[0];
                secondKnot.point.m_Pos = pos;
                secondKnot.point.m_Orient = ori;
                secondKnot.point.m_Pressure = pressure;
                secondKnot.point.m_TimestampMs = (uint)(Time.time * 1000);
                secondKnot.smoothedPos = pos;
                m_knots.Add(secondKnot);
                
                // Don't mark control points as changed yet - wait for actual movement
                // m_FirstChangedControlPoint = 0; // Removed this line
                return false; // Don't add another knot yet
            }
            ...
            updated.point.m_TimestampMs = (uint)(Time.time * 1000);
            ...
            // Only mark control points as changed if there's meaningful movement
            float moveDistance = Vector3.Distance(m_knots[iUpdate].point.m_Pos, pos);
            if (moveDistance > 0.001f) // Only mark as changed if there's actual movement
            {
                m_HasHadMeaningfulMovement = true; // Mark that we've had meaningful movement
                ...
                {
                    // For the first meaningful movement, mark both knots as changed
                    m_FirstChangedControlPoint = 0; // Start from the beginning
                }
            }
        ...
        protected void ApplySmoothing(Knot prev, ref Knot next)
        {
            ...
            float window = pressureSmoothWindowMeters_PS * (SimpleAppConfig.Instance?.MetersToUnits ?? 1.0f) * POINTER_TO_LOCAL;
