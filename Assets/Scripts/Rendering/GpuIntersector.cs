...
namespace TiltBrush
{
    ...
    // Simplified GPU intersection system for lightweight drawing.
    //
    // This class provides basic GPU-accelerated intersection detection for brush strokes.
    // Simplified for lightweight systems without complex widget management.
    //
    public class GpuIntersector : MonoBehaviour
    {
        ...
        {
            // Simplified: no widget support for lightweight system
        ...
        private void Start()
        {
            // Simplified: Use Unity's built-in application quit event
            Application.quitting += OnGuaranteedAppQuit;

            // Compute kernel (blit tex -> buffer) setup.
            if (m_ComputeCopyShader != null)
            {
            ...
            Vector3 vDetectionCenter_GS, float radius_GS, int renderCullingMask)
        {
            ...
            // Simplified: Use main camera rotation for intersection camera orientation
            m_IntersectionCamera.transform.rotation = Camera.main.transform.rotation;
            ...
            if (SimpleAppConfig.Instance.GeometryShaderSuppported)
            {
            ...
            override protected void OnReadResults()
            {
                ...
                for (int i = 0; i < resultColors.Length; i++)
                {
                    ...
                    if (c > 0)
                    {
                        ...
                        batch = SimpleApp.Instance?.ActiveCanvas?.BatchManager?.GetBatch(batchId);
                        if (batch != null)
                        {
                        ...
                        {
                            // Simplified: No widget support in lightweight system
                            continue;
                        }
                        ...
                        {
                            Debug.LogWarningFormat(
                                "Unexpected: Null batch for batchId {0}",
                                batchId);
                            continue;
                        }

                        m_ResultList.Add(
                            new BatchResult { subset = subset });
                        m_ResultCount++;
                    }
                }
            ...
            override protected void OnReadResults()
            {
                ...
                for (int i = 0; i < resultColors.Length; i++)
                {
                    ...
                    if (c > 0)
                    {
                        ...
                        ushort batchId = (ushort)((c & 0xffff0000) >> 16);
                        // Simplified: No widget support in lightweight system
                        // Just mark that we found an intersection
                        m_ResultList.Add(new ModelResult { hasIntersection = true });
                        m_ResultCount++;
                    }
                }
