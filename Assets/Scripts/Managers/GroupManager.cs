...
namespace TiltBrush
{
    ...
    public class GroupManager
    {
        ...
        public static void MoveStrokesToNewGroups(List<Stroke> strokes, Dictionary<int, int> oldGroupToNewGroup, GroupManager groupManager = null)
        {
            ...
            foreach (var stroke in strokes ?? new List<Stroke>())
            {
                if (stroke.Group != SketchGroupTag.None)
                {
                    ...
                    {
                        // Use provided groupManager or create a fallback group
                        var newGroup = groupManager?.NewUnusedGroup() ?? new SketchGroupTag(1);
